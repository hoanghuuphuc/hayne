						...
						<form action="/admin/bill/${ o.id }" method="post">
							<select class="form-control" onchange="this.form.submit()" name="stt"
							 ${ o.status >= 3 ? 'style="color: green;" disabled' : ''}>
								...
								<option value="3" ${ o.status == 3 ? 'selected' : ''}${ o.status < 2 ? 'disabled' : ''} style="color: lime;">Đã Giao Hàng</option>
								<option value="4" ${ o.status == 4 ? 'selected' : ''} style="color: lime;">Đã Hủy </option>
							</select>
						</form>
					</td>
